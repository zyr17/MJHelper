...
class Algo{
    ...
    static calcyaku(data){
        ...
        function pinfu(data){
            if (data.pinfu && data.naki.length == 0) return 1;
            return 0;
        }
        ...
        function chinroutou(data){
            ...
            for (var i = 0; i < str.length; i += 2){
                var num = tile2num[str.slice(i, i + 2)];
                if (!(num % 10 == 0 || num % 10 == 9) || str.slice(i + 1, i + 2) == 'z')
                    return 0;
            }
            return 13;
        }
    ...
    static calcfu(data){
        ...
        if (data.naki.length == 0 && data.tsumo == false)
            res.push(['menchinron', 10]);
        if (data.tsumo == true)
            res.push(['tsumo', 2]);
        var tsumofu = res.length;
        ...
        if (res.length == tsumofu) data.pinfu = true;
        else data.pinfu = false;
        return res;
    }

    static calcyakufuone(data){
        var funame = this.calcfu(data);
        let fu = 0;
        for (let i = 0; i < funame.length; i ++ )
            fu += funame[i][1];
        var result = this.calcyaku(data);
        result.fu = fu;
        result.funame = funame;
        var tmpname = {};
        for (var i = 0; i < result.yakuname.length; i ++ )
            tmpname[result.yakuname[i].name] = 1;
        result.calcfu = result.fu;
        if ('chitoitsu' in tmpname) {
            result.fu = 25;
            result.funame = [['chitoi', 25]];
        }
        else if ('tsumo' in tmpname && 'pinfu' in tmpname) {
            result.fu = 20;
            result.funame = [['tsumopinfu', 20]];
        }
        else{
            if (result.fu == 20) {
                result.fu = 30;
                result.funame = [['kuipinfu', 30]];
            }
    ...
    static calctensu(data, oya, tsumo, honba = 0){
    ...
    static calcyakufu(data){
        ...
        var result = {'yaku': 0, 'fu': 0, 'yakuname': [], resulttitle: '', 'funame': []};
        ...
        for (var i = 0; i < tehais.length; i ++ ){
            ...
            var onetensu = this.calctensu(oneres, data.jifu == 0, data.tsumo);
            if (onetensu.tokuten + oneres.yaku > resulttensu.tokuten + result.yaku){
                resulttensu = onetensu;
                result = oneres;
            }
        }
        var one = result.fu * Math.pow(2, result.yaku + 2);
        if (result.yaku == 0) result.resulttitle = 'chonbo'
        if (one >= 2000 || result.yaku == 5) result.resulttitle = 'mangan';
        if (result.yaku > 5) result.resulttitle = 'haneman';
        if (result.yaku > 7) result.resulttitle = 'baiman';
        if (result.yaku > 10) result.resulttitle = 'sanbaiman';
        if (result.yaku > 12) result.resulttitle = parseInt(result.yaku / 13) + 'yakuman';
