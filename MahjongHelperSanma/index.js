...
let default_settings = {
    ...
    reachcost: 1000,
    honbacost: 100,
    initscore: [35000, 35000, 35000],
    backscore: 35000,
    uma: [10, 0, -10],
    historylength: 1000,
    tenpaigets: [0, 2000, 1000],
    tenpaigives: [0, 2000, 1000],
    showhint: true,
    tsumosun: true,
    yamanum: 4,
...
try {
    stored_logs = localStorage.getItem('sanma_logs');
    stored_settings = localStorage.getItem('sanma_settings');
}
catch {
...
const VueApp = {
    ...
    methods: {
        savetable() {
            ...
            if (have_localstorage) localStorage.setItem('sanma_logs', JSON.stringify(this.logs));
        },
        savesettings() {
            if (have_localstorage) localStorage.setItem('sanma_settings', JSON.stringify(this.settings));
        },
        cleartable() {
        ...
        initround() {
            for (let i = 0; i < this.players.length; i ++ ) {
                ...
                this.players[i].kaze = (i + this.players.length - this.kyoku) % this.players.length;
            }
        },
        ...
        agariconfirm() {
            ...
            let oya = (this.players.length + this.kyoku - this.settings.agariposition) % this.players.length;  // oya relative player number
            let res = [0];
            if (this.agaricolor[0]) {
                ...
                for (let i = 0; i < this.agaritoplayer.length; i ++ )
                    if (this.agaritoplayer[i] == -1 && oya >= i) oya ++ ;  // relative player number to relative position number, skip north so add 1
                console.log(this.agaritoplayer, oya, this.agaricolor);
                let roncount = 0;
                for (let i = 1; i < this.agaricolor.length; i ++ )
                    if (this.agaritoplayer[i] != -1)
        ...
        manualpointclick() {
            let points = [];
            for (let i = 0; i < this.settings.manualpoints.length; i ++ )
                points.push(this.settings.manualpoints[(i - this.kyoku + this.players.length) % this.players.length]);
    ...
    computed: {
        ...
        agarivalidposition() {
            let res = [];
            for (let i = 0; i < this.settings.positionname.length; i ++ )
                res.push((i + 1) % 4 != (4 - this.settings.agariposition) % 4);
            return res;
        },
        agaritoplayer() {
            let res = [];
            let now = this.settings.agariposition;
            for (let i = 0; i < this.settings.positionname.length; i ++ )
                if ((i + 1) % 4 != (4 - this.settings.agariposition) % 4) {
                    res.push(now);
                    now = (now + 1) % this.players.length;
                }
                else
                    res.push(-1);
            return res;
        ...
        dicearrowpos() {
            if (this.settings.yamanum == 4)
                return this.settings.positionname[(this.settings.dices[0] + this.settings.dices[1] + 1) % 4];
            let relative = (this.settings.dices[0] + this.settings.dices[1] - 1) % 3;
            if (relative + this.kyoku >= 3) relative ++ ;
            relative = (relative + 2) % 4;
            return this.settings.positionname[relative];
        }
    }
}
